#!/usr/bin/env python3
"""
AIFFFile class for handling AIFF audio files with ID3 tags
"""
...
class AIFFFile(AudioFileBase):
    """Handles AIFF files with ID3 tags"""
    
    SUPPORTED_EXTENSIONS = {'.aiff', '.aif'}
    
    def load(self):
        """Load the AIFF file and extract tags"""
        try:
    ...
    def _get_tag(self, tag_name: str) -> str:
        """Get a tag value from the AIFF file"""
    ...
    def save_tags(self, new_tags: Dict[str, str]):
        """Save updated tags to the AIFF file"""
        try:
            # AIFF uses ID3 tags
